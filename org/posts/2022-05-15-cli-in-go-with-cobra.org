...
Another quick search, =`go cross compile arm darwin`=, and [[https://sodocumentation.net/go/topic/1020/cross-compilation][bam]]! Magic table, pretty much what I expected -- either =`arm`= or =`arm64`= in another variable.
...
* Fin
I'd say it's about time to actually start cracking into what this CLI is going to do! That's a much less topic to blog about, so I'll end things here.
